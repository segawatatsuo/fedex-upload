        ...
        <div class="row">
            @foreach ($premium_silks as $ps)
                <div class="col-sm-15 col-6">
                    ...
                    <div>＄{{ number_format( $ps->price, 2 ) }} </div>

                    <div class="caption1">CARTON</div>

                    @if ($ps->stock == '0')
                        <input type="text" id="{{ $ps->product_code }}" class="txtCal outofstock"
                            name="item[{{ $ps->product_code . ' | ' . $ps->title_header . ' | ' . $ps->price }}]"
                            value="" placeholder="Out of stock"  readonly>
                    @else
                        <input type="text" id="{{ $ps->product_code }}" class="txtCal"
                            name="item[{{ $ps->product_code . ' | ' . $ps->title_header . ' | ' . $ps->price }}]"
                            value="" placeholder="Enter the number">
                    @endif

                    <div class="caption1">PCS</div>
                    <input type="text" readonly style="border-width:1px" type="text" class="border-top-0 border-right-0 border-left-0" id="{{ $ps->product_code . '-PCS' }}" value=""
                        placeholder="">
                </div>
            @endforeach
        </div>
        ...
        <div class="row">
            ...
            @foreach ($diamond_legs as $dl)
                <div class="col-sm-15 col-6">
                    ...
                    <div>＄{{ number_format( $dl->price , 2 ) }} </div>

                    <div class="caption1">CARTON</div>
                    @if ($dl->stock == '0')
                    <input type="text" id="{{ $dl->product_code }}" class="txtCal outofstock"
                        name="item[{{ $dl->product_code . ' | ' . $dl->title_header . ' | ' . $dl->price }}]" value=""
                        placeholder="Out of stock" readonly>
                    @else
                    <input type="text" id="{{ $dl->product_code }}" class="txtCal"
                        name="item[{{ $dl->product_code . ' | ' . $dl->title_header . ' | ' . $dl->price }}]" value=""
                    ...
                    <div class="caption1">PCS</div>
                    <input type="text" readonly style="border-width:1px" type="text" class="border-top-0 border-right-0 border-left-0" id="{{ $dl->product_code . '-PCS' }}" value=""
                        placeholder="">
